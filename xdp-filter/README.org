...
** -p, --policy <policy>
This sets the policy =xdp-filter= applies to packets matched by the filter
rules. The valid values are:

 * *allow* (default): Matched packets are dropped (=XDP_DROP=), unmatched packets pass through.
 * *deny*: All packets are dropped except those matched by the filter rules.
 * *tx*: Matched packets are bounced back out the ingress interface (=XDP_TX=), unmatched packets pass through.

=xdp-filter= cannot be loaded simultaneously in different policy modes on the
system. Unload xdp-filter before switching to a different policy mode.
 ...
 * *ethtype*: Support filtering on EtherType (Ethernet protocol type)
...
* The ETHTYPE command
Use the =ethtype= command to add an EtherType (Ethernet protocol type) to the
=xdp-filter= match list. For this to work, =xdp-filter= must be loaded with the
*ethtype* feature on at least one interface.

The syntax for the =ethtype= command is:

=xdp-filter ethtype [options] <ethtype>=

Where =<ethtype>= is the EtherType value in hexadecimal to add (or remove if the
*--remove* is specified). Common values include =0800= (IPv4), =0806= (ARP), and
=86dd= (IPv6). The supported options are:

** -r, --remove
Remove the EtherType instead of adding it.

** -s, --status
If this option is specified, the current list of matched EtherTypes will be
printed after inserting the EtherType. Otherwise, nothing will be printed.
...
To bounce all traffic arriving on port 80 back out the interface (TX mode):

#+begin_src sh
# xdp-filter load eth0 -f tcp,udp -p tx
# xdp-filter port 80
#+end_src

To filter all IPv4 traffic by EtherType:

#+begin_src sh
# xdp-filter load eth0 -f ethtype
# xdp-filter ethtype 0800
#+end_src

To bounce all ARP traffic back out the interface using TX mode:

#+begin_src sh
# xdp-filter load eth0 -f ethtype -p tx
# xdp-filter ethtype 0806
#+end_src
